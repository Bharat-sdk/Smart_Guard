...
class FragmentActivityHistory:BaseFragment<ActivityHistoryViewModel,FragmentActivityHistoryBinding>() {
    ...
    override fun initView() {
        ...
        val groupedMapMap: Map<String, List<ActivityHistoryList>> = AppLists(requireContext()).activityList.groupBy {
        ...
        for (date:String in groupedMapMap.keys){
            ...
            groupItems?.forEach {
                consolidatedList.add(ActivityHistoryList(it.activity_history_time_stamp,it.activity_history_message,it.hub_id))
            }
        }
